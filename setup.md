...
```bash
# Cấu hình Nginx
cat > /etc/nginx/sites-available/utgachoi.com << 'EOF'
server {
    listen 80;
    server_name utgachoi.com www.utgachoi.com;
...
# Kích hoạt cấu hình
ln -sf /etc/nginx/sites-available/utgachoi.com /etc/nginx/sites-enabled/
rm -f /etc/nginx/sites-enabled/default
nginx -t
systemctl restart nginx

# Cài đặt SSL với Let's Encrypt
certbot --nginx -d utgachoi.com -d www.utgachoi.com --non-interactive --agree-tos --email caoleanhcuong78@gmail.com
```
...
RECORDING_VIDEO_URL_BASE=https://utgachoi.com/videos
RECORDING_THUMBNAIL_URL_BASE=https://utgachoi.com/videos/thumbnails
RECORDING_RETENTION_DAYS=3

# Frontend Build Args 
VITE_API_URL=https://utgachoi.com/api
VITE_WS_URL=wss://utgachoi.com/ws/chat
VITE_HLS_BASE_URL=https://utgachoi.com/live
EOL
```
...
export DOCKER_REGISTRY=ghcr.io/YOUR_USERNAME/livestream  # Thay YOUR_USERNAME bằng username GitHub của bạn
export TAG=latest
...
3. Kiểm tra website:
   - Truy cập https://utgachoi.com
   - Kiểm tra API: https://utgachoi.com/api/swagger-ui.html
   - Kiểm tra health: https://utgachoi.com/api/actuator/health
